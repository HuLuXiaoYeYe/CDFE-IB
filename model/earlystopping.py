...
class EarlyStopping:
    ...
    Args:
            ...
            trace_func (function): trace print function.
                            Default: print
    '''
    def __init__(self, patience=30, verbose=False, delta=0,  trace_func=print, tmpFile=io.BytesIO()):
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        if self.verbose:
            logging.info(f'validate metric increased {self.val_score:.6f} --> {val_loss:.6f}).  Saving model ...')
        if isinstance(self.tmpFile, io.IOBase):
            self.tmpFile.seek(0)
